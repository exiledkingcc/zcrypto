...
#endif
#elif !defined(__ZCRYPO_BYTE_ORDER__)
...
static inline uint32_t _z_load_le_u32(const uint8_t bs[4]) {
    return *(uint32_t*)bs;
}

static inline uint32_t _z_load_be_u32(const uint8_t bs[4]) {
    return ((uint32_t)bs[0] << 24) | ((uint32_t)bs[1] << 16) | ((uint32_t)bs[2] << 8) | bs[3];
}

static inline void _z_store_le_u32(const uint32_t x, uint8_t bs[4]) {
    *(uint32_t*)bs = x;
}

static inline void _z_store_be_u32(const uint32_t x, uint8_t bs[4]) {
    bs[0] = (x >> 24) & 0xff;
    bs[1] = (x >> 16) & 0xff;
    bs[2] = (x >> 8) & 0xff;
    bs[3] = (x)&0xff;
...
static inline void _z_store_le_u64(const uint64_t x, uint8_t bs[8]) {
    *(uint64_t*)bs = x;
}

static inline void _z_store_be_u64(const uint64_t x, uint8_t bs[8]) {
    bs[0] = (x >> 56) & 0xff;
    bs[1] = (x >> 48) & 0xff;
    bs[2] = (x >> 40) & 0xff;
    bs[3] = (x >> 32) & 0xff;
